...
const buildConditionNotes = (lines, fields) => {
  ...
  if (conditionHeading !== -1) {
    ...
    for (let index = conditionHeading + 1; index < lines.length; index += 1) {
      ...
      if (!line || isInternalLine(line) || !isPropertyConditionLine(line)) continue;
  ...
  const labeledFacts = [
    ...
    .map(([labels, prefix]) => {
      ...
      return value && !isInternalLine(value) ? sentence(`${prefix}${value}`) : '';
    })
    .filter(Boolean);
...
export const parseDealInput = (rawInput) => {
  ...
  const addressValue = findField(fields, ['address']) || text.match(/^address\s*:?\s*(.+)$/im)?.[1] || '';
  if (addressValue) {
...
export const getMissingDealFields = (deal) => REQUIRED_FIELDS
  .filter(([key]) => !String(deal[key] || '').trim())
  .map(([, label]) => label);
