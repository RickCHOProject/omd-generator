...
export const buildConditionNoteOptions = (value) => {
  const polished = polishConditionNotes(value);
  if (!polished) return [];
  ...
  return [
    {
      ...
      description: 'Positive and professional, with the full improvement scope clearly disclosed.',
      text: joinConditionSections(
        hasImprovementScope
          ? 'This property offers a clear value-add opportunity for an investor with a plan for the identified improvements.'
          : 'The available property-condition details are summarized below.',
        improvements,
        highlights,
        verification
      )
    ...
    {
      id: 'value-add',
      label: 'Value-Add Focus',
      description: 'Leads with the upside while keeping every known repair and due-diligence item.',
      text: joinConditionSections(
        hasImprovementScope
          ? 'Investors looking for a renovation opportunity can add value by completing the documented improvement scope.'
          : 'The property includes the following condition highlights.',
        highlights.length ? ['Property highlights:', ...highlights] : [],
        improvements.length ? ['Improvement scope:', ...improvements] : [],
        verification.length ? ['Buyer due diligence:', ...verification] : []
      )
    },
    {
      id: 'direct',
      label: 'Direct & Professional',
      description: 'A concise factual version for experienced investors who prefer the scope upfront.',
      text: joinConditionSections(
        improvements.length ? ['Known improvement items:', ...improvements] : [],
        highlights.length ? ['Property highlights:', ...highlights] : [],
        verification.length ? ['Items to verify:', ...verification] : []
      ) || polished
    }
  ];
};
