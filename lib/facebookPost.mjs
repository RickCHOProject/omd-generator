const FACEBOOK_VARIANTS = [
  {
    ...
    cta: ({ phone }) => `Text ${phone} if you are interested or have questions.`
  },
  {
    ...
    cta: ({ phone }) => `Want to take a closer look? Text ${phone}.`
  },
  {
    ...
    cta: ({ phone }) => `For questions or showing availability, text ${phone}.`
  },
  {
    ...
    cta: ({ phone }) => `Interested buyers can text ${phone} for more information.`
  },
  {
    ...
    cta: ({ phone }) => `Take a look and text ${phone} if it fits what you are buying.`
  },
  {
    ...
    cta: ({ phone }) => `Questions about the opportunity? Text ${phone}.`
  }
];
...
export const buildFacebookPost = (deal, options = {}) => {
  ...
  const dealUrl = normalizeText(options.dealUrl, '');
  ...
  if (dealUrl) {
    sections.push(`See the photos and full deal details:\n${dealUrl}`);
  } else {
    sections.push('The full deal link will be added automatically after the property is published.');
  }
